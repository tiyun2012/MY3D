    ...
    switch (viewMode)
    {
        ...
        case ViewMode::Far:
            position =
                target +
                TiMath::Vector3(
                    distance * std::cos(pitchDegrees * TiMath::PI / 180.0f) *
                        std::sin(yawDegrees * TiMath::PI / 180.0f),
                    distance * std::sin(pitchDegrees * TiMath::PI / 180.0f),
                    distance * std::cos(pitchDegrees * TiMath::PI / 180.0f) *
                        std::cos(yawDegrees * TiMath::PI / 180.0f));
...
TiMath::Matrix4 Camera::getProjectionMatrix() const
{
    ...
    {
        return TiMath::Matrix4::perspective(fovYDegrees, aspectRatio, zNear,
                                            zFar);
    }
    float halfHeight = distance;
    float halfWidth = halfHeight * aspectRatio;
    return TiMath::Matrix4::orthographic(-halfWidth, halfWidth, -halfHeight,
...
void Camera::focusOnPoint(const TiMath::Vector3& point, float distance)
{
    ...
    // For other view modes, getViewMatrix will compute position based on target
    // and distance
}

void Camera::orbitAroundPoint(const TiMath::Vector3& point, float deltaX,
...
TiMath::Vector3 Camera::projectToArcball(float mouseX, float mouseY,
                                         float width, float height,
                                         const TiMath::Vector3& center,
                                         float radius)
{
    // Normalize to [-1, 1]
...
void Camera::startArcball(const TiMath::Vector3& point, float mouseX,
                          float mouseY, float width, float height)
{
    arcballActive = true;
    arcballStartCamPos = getPosition();
    arcballStartVec =
        projectToArcball(mouseX, mouseY, width, height, point, arcballRadius);
}

void Camera::updateArcball(const TiMath::Vector3& point, double mouseX,
                           double mouseY, double width, double height)
{
    if (!arcballActive) return;
    TiMath::Vector3 currVec =
        projectToArcball(mouseX, mouseY, width, height, point, arcballRadius);
    ...
    TiMath::Quaternion q =
        TiMath::Quaternion::fromAxisAngle(axis.normalized(), angle);
...
void Camera::endArcball() { arcballActive = false; }
...
TiMath::Vector3 Camera::getPosition() const
{
    ...
    float radPitch = pitchDegrees * TiMath::PI / 180.0f;
    return target +
           TiMath::Vector3(distance * std::cos(radPitch) * std::sin(radYaw),
