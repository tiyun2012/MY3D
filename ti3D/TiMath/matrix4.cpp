...
Matrix4 Matrix4::lookAt(const Vector3& eye, const Vector3& target, const Vector3& up) {
    ...
    if (dir.isZero()) {
        throw std::runtime_error("Zero-length direction in Matrix4::lookAt");
    }
    Vector3 upNorm = up.normalized();
    if (upNorm.isZero()) {
        throw std::runtime_error("Zero-length up vector in Matrix4::lookAt");
    }
    Vector3 right = dir.cross(upNorm).normalized();
    if (right.isZero()) {
        throw std::runtime_error("Invalid up vector in Matrix4::lookAt");
    }
...
Matrix4 Matrix4::inverse() const {
    ...
    result.m[9] = (-a00 * b01 + a02 * b05 - a03 * b05) * invDet;
    ...
    result.m[11] = (-a00 * b01 + a01 * b05 - a02 * b05) * invDet;
